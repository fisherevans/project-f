...
- Skills trigger abilities on a **shared tick timeline**, each combatants' skills activating in parallel.
...
## Load-outs, Skills & Ticks

A load-out is a set of 4 skills a player has chosen before beginning a run. These are the only skills they have during that run. While in combat, these 4 skills are available to be used. There is no explicit cooldown or inherent cost to skills.

> Some powerful skills might require a certain level of focus, which is a replenishing resource introduced later on in the game. If one of these skills activates while the player doesn't have enough focus, the skill will backfire.

- Each **skill** has a fixed number of ticks (at least 2).  
- Each tick can do nothing, or many things:
  - Deal damage
  - Apply status stacks
  - Enter or leave a stance
  - etc.
- Example: An 8-tick skill may:  
  - Tick 1: Deal light damage  
  - Ticks 2–3: Idle  
  - Ticks 4–6: Enter a defensive stance  
  - Tick 7: Idle  
  - Tick 8: Deal heavy damage + apply a status

- Because ticks alternate globally (player → enemy → player → enemy), careful timing is critical to avoid being vulnerable when the opponent attacks, and to take advantage of an opponents state.

## Stances

Stances are **temporary combat states** tied to a sequence of ticks of a skill.  
...
- Each status has **stacks** and a **level (1–3)** (based on thresholds of the stack)  
- Each tick, stacks are reduced according to rules specific to that status type.
...
Statuses are thematically aligned with the game’s five combat **types**.
...
While not a rock-paper-scissors system, the five types define the **themes and behaviors** of skills and statuses:  Kinetic, Thermal, Corrosive, Voltaic, and Growth. More akin to Magic the Gathering affinities. See [Types](types.md) for more details.
...
- Swap out skills in your load-out
