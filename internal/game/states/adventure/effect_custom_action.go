...
func convertCustomActionStep(params any, tc *TemplateContext, sequences map[string]*SequenceDef) []Effect {
	...
	return []Effect{NewFunctionEffect(func(s *State) {
		...
		// Build params fresh each invocation to avoid sharing across parallel calls
		...
		childEffects := convertSteps(actionDef.Steps, childTC, sequences)
		if len(childEffects) > 0 {
			s.planExecutor.StartPlan(nil, NewSerialPlan(childEffects...))
		}
	})}
}
